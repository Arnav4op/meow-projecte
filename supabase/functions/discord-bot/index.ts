...
async function getActiveDispatch(supabase: any, userId: string) {
  const { data } = await supabase
    ...
    .select(`
      ...
      route:routes(flight_number, departure_airport, arrival_airport, distance_nm)
    `)
    .eq('user_id', userId)
    .eq('status', 'assigned')
...
async function getUserTypeRatings(supabase: any, userId: string) {
  const { data } = await supabase
    ...
    .select(`
      id,
      is_active,
      aircraft:aircraft(id, name, type_code, family, seats, price)
    `)
    .eq('user_id', userId)
  return data || []
}
...
async function processPayment(supabase: any, fromUserId: string, toCallsign: string, amount: number) {
  ...
  await supabase
    .from('profiles')
    .update({ money: sender.money - amount })
    .eq('user_id', fromUserId)
  
  await supabase
    .from('profiles')
    .update({ money: recipient.money + amount })
...
async function linkDiscordAccount(supabase: any, discordId: string, discordUsername: string, email: string) {
  ...
  if (profile.discord_id && profile.discord_id !== discordId) {
    return { error: 'This account is already linked to another Discord user' }
  }
...
async function getLeaderboard(supabase: any, limit = 10) {
  const { data: profiles } = await supabase
    .from('profiles')
    .select('callsign, name, total_hours, total_flights, money')
...
async function handleCommand(interaction: any): Promise<DiscordResponse> {
  ...
  switch (commandName) {
    case 'ping': {
      return {
        type: 4,
        data: createEmbed('🏓 Pong!', 'AFLV Discord Bot is online and operational!', 0x22c55e)
      }
    }
    ...
    case 'link': {
      ...
      if (!email) {
        return {
          type: 4,
          data: createEmbed('🔗 Link Account', 'Please provide your email: `/link email:pilot@aeroflot.ru`', 0xeab308)
        }
      }
      ...
      if (result.error) {
        return {
          type: 4,
          data: createEmbed('❌ Error', result.error, 0xef4444)
        }
      }
      ...
      return {
        type: 4,
        data: {
          ...createEmbed('✅ Account Linked!', `Your Discord account has been linked to **${email}**\n\nUse \`/profile\` to view your pilot profile!`, 0x22c55e),
          components: []
        }
      }
    }
    ...
    case 'profile': {
      if (!user) {
        return {
          type: 4,
          data: {
            ...createEmbed('❌ Not Linked', 'Please link your account first using `/link callsign:YOURCALLSIGN`', 0xeab308),
            components: []
          }
        }
      }
      ...
      if (!stats) {
        return {
          type: 4,
          data: createEmbed('❌ Error', 'Could not load profile', 0xef4444)
        }
      }
      ...
      return {
        ...
        data: createEmbed(
          '👤 Pilot Profile',
          `**${stats.profile.callsign}** - ${stats.profile.name}`,
          0x06b6d4,
          [
    ...
    case 'my-dispatch': {
      if (!user) {
        return {
          type: 4,
          data: {
            ...createEmbed('❌ Not Linked', 'Please link your account first using `/link`', 0xeab308),
            components: []
          }
        }
      }
      ...
      if (!dispatch) {
        const { data: careerRequest } = await supabase
        ...
        if (careerRequest) {
          return {
            type: 4,
            data: createEmbed(
              '⏳ Career Request Pending',
              'Your career request is awaiting admin approval.',
              0xeab308
            )
          }
        }
        ...
        if (purchasedAircraft.length === 0) {
          return {
            type: 4,
            data: {
              ...createEmbed(
                '🎖️ No Active Dispatch',
                'You don\'t have an active dispatch. Purchase aircraft from the shop to start your career!',
                0x3b82f6
              ),
              components: []
            }
        ...
        return {
          ...
          data: {
            ...createEmbed(
              '🎖️ Request Career',
              `Current Aircraft Family: **${currentFamily}**\n\nSelect an aircraft family to request a career dispatch:`,
              0x3b82f6
            ),
            ...createSelectMenu(
              'select_aircraft_family',
              'Select Aircraft Family',
              purchasedAircraft.map((ac: any) => ({
                label: `${ac.name} (${ac.family})`,
                value: ac.id,
                description: `Price: $${ac.price?.toLocaleString()}`
              }))
            )
          }
        }
      ...
      return {
        ...
        data: createEmbed(
          '✈️ Active Dispatch',
          `**Leg ${dispatch.leg_number}** - ${dispatch.status}`,
          0x22c55e,
          [
    ...
    case 'pirep': {
      if (!user) {
        return {
          type: 4,
          data: createEmbed('❌ Not Linked', 'Please link your account first', 0xeab308)
        }
      }
      ...
      if (Pireps.length === 0) {
        return {
          type: 4,
          data: createEmbed('📝 No PIREPs', 'You haven\'t submitted any PIREPs yet.', 0x3b82f6)
        }
      }
      ...
      return {
        type: 4,
        data: createEmbed('📝 Recent PIREPs', 'Your last 5 flight reports:', 0x06b6d4, fields)
      }
    }
    ...
    case 'shop': {
      ...
      if (available.length > 0) {
        ...
        available.slice(0, 10).forEach((ac: any) => {
          description += `${ac.name} - $${ac.price?.toLocaleString()}\n`
        })
      }
      ...
      return {
        ...
        data: {
          ...
          components: owned.length > 0 ? [
            {
              type: 1,
              components: [{
                type: 3,
                custom_id: 'set_active_aircraft',
                placeholder: 'Set Active Aircraft',
                options: owned.map((ac: any) => ({
                  label: ac.name,
                  value: ac.id
                }))
              }]
    ...
    case 'bank': {
      ...
      if (!stats) {
        return {
          type: 4,
          data: createEmbed('❌ Error', 'Could not load bank data', 0xef4444)
        }
      }
      ...
      return {
        ...
        data: {
          ...createEmbed(
            '💰 Bank Account',
            `**Balance:** $${user.money?.toLocaleString() || 0}`,
            0x22c55e,
            [
          ...
          ...createButtons([
            { label: '💸 Pay Pilot', style: 2, custom_id: 'pay_button' }
          ])
        }
      }
    }
    ...
    case 'pay': {
      ...
      if (!callsign || !amount) {
        return {
          type: 4,
          data: createEmbed('💸 Payment', 'Usage: `/pay callsign:AFLV021 amount:10000`', 0xeab308)
        }
      }
      ...
      if (result.error) {
        return {
          type: 4,
          data: createEmbed('❌ Payment Failed', result.error, 0xef4444)
        }
      }
      ...
      return {
        ...
        data: createEmbed(
          '✅ Payment Successful!',
          `Sent **$${amount.toLocaleString()}** to **${callsign}**\n\nYour new balance: $${(result.sender.money - amount).toLocaleString()}`,
          0x22c55e
        )
      }
    }
    ...
    case 'leaderboard': {
      ...
      return {
        type: 4,
        data: createEmbed('🏆 Leaderboard', 'Top pilots by flight hours:', 0xeab308, fields)
      }
    }
    ...
    case 'fleet': {
      ...
      if (fleet.length === 0) {
        return {
          type: 4,
          data: createEmbed('✈️ Virtual Fleet', 'No aircraft in fleet', 0x3b82f6)
        }
      }
      
      const fields = fleet.slice(0, 10).map((f: any) => ({
        name: f.tail_number,
        value: `${f.aircraft_type} | ${f.status} | ${f.current_location}`,
        inline: true
      }))
      
      return {
        type: 4,
        data: createEmbed('✈️ Virtual Fleet', 'Aircraft status:', 0x06b6d4, fields)
      }
    }
    ...
    case 'stats': {
      ...
      if (!stats) {
        return {
          type: 4,
          data: createEmbed('❌ Error', 'Could not load statistics', 0xef4444)
        }
      }
    ...
    default: {
      return {
        type: 4,
        data: createEmbed('❓ Unknown Command', 'Use `/help` for available commands', 0xef4444)
      }
    }
  }
}
  ...
  switch (customId) {
    case 'pay_button': {
      return {
        ...
        data: createEmbed(
          '💸 Make Payment',
          'Use `/pay callsign:AFLV021 amount:10000` to pay another pilot.',
          0x3b82f6
        )
      }
    }
    ...
    case 'select_aircraft_family': {
      if (!user) {
        return {
          type: 4,
          data: createEmbed('❌ Error', 'Please link your account first', 0xef4444)
        }
      }
      ...
      return {
        type: 4,
        data: createEmbed(
          '✅ Career Request Submitted!',
          'Your career request has been submitted for review. You\'ll be notified once approved!',
          0x22c55e
        )
      }
    }
    ...
    case 'set_active_aircraft': {
      ...
      return {
        type: 4,
        data: createEmbed(
          '✅ Active Aircraft Updated!',
          'Your active aircraft has been changed.',
          0x22c55e
        )
      }
    }
    ...
    default: {
      return {
        type: 4,
        data: createEmbed('❓ Unknown', 'Action not recognized', 0xef4444)
      }
    }
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    const interaction: any = await req.json()
    
    if (interaction.type === 1) {
      return new Response(JSON.stringify({ type: 2 }), {
